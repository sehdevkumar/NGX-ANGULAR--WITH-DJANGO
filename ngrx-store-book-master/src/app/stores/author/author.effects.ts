...
export class AuthorEffects {
  ...
  postAuthors$ = createEffect((): any => {
    return this.actions$.pipe(
      ...
      switchMap((action) =>
        this.authorService?.onPostAuthor(action.postAuhtor).pipe(
          map(() => {
  ...
  getAuthors$ = createEffect((): any => {
    return this.actions$?.pipe(
      ofType(getAuthors),
      switchMap(() => {
        return this.authorService?.onGetAllAuthors().pipe(
          map((data) => {
  ...
  deleteAuthors$ = createEffect((): any => {
    return this.actions$?.pipe(
      ...
      switchMap((action) => {
        return this.authorService?.onDeleteAuthor(action.postAuthor).pipe(
          map((data) => {
            return getAuthors()
          }),
          catchError((error): any => {
